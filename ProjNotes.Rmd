...
data[[2]][grep("LOC113218526", data[[2]]$Gene),]
```
...
If there are any issues (as there has been as of now, 12th December, where my laptop shut down unexpectedly and I lost all environment objects), it is not feasible to run this again. Instead I will read in boot.results from the results file already produced.

```{r}
#boot.results <- read.table("output/BootStrapResults.tsv", header = T)
#boot.results
```
