...
Only 8.9% are to be completely considered uncertain ? I mean that is great if that's the case but may be worth a quick manual check.

```{r}
nrow(data.sig[data.sig$TwoHitCheck == "Fail",])
nrow(data.sig[data.sig$ThreeHitCheck == "Fail",])
nrow(data.sig[data.sig$TwoHitCheck == "Fail" | data.sig$ThreeHitCheck == "Fail",])
nrow(data.sig)
```
