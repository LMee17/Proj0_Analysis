...
In order to test proprtions between lineages I'm going to test them simultaneously using r's 'prop.test' function. I can't find much on what test it actually runs, but it seems that it uses Pearson's Chisq, but without pairwise comparisons. 
...
AllComplex and AllOrigin have have 28 more genes in their dataframes than AllSol. I hope this is down to duplications.

```{r}
nrow(allcom[!duplicated(allcom$Gene),])
```
...
Ok now they're all 6109, hopefully all the same.
...
```{r}
nrow(allcom[allcom$adj_pvalue < 0.051,]) == nrow(all.list2[[2]][all.list2[[2]]$Selection == 1,])
```

Make really sure

```{r}
nrow(allsol[allsol$adj_pvalue < 0.051,]) == nrow(all.list2[[1]][all.list2[[1]]$Selection == 1,])
```
