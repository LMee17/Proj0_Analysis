...
can.results <- lapply(X=data, y=100000, FUN = can.assess)
can.results <- bind_rows(can.results)
...
non.results <- lapply(X=data, y=100000, FUN = non.assess)
non.results <- bind_rows(non.results)

#Create one table
boot.results <- rbind(can.results, non.results)
boot.results
