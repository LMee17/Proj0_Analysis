...
adj.results <- lapply(data, fisher.craft.adj)

#Combine into a table
#Record tests
Test <- rownames(adj.results[[1]])

#Unlist and add test names as first column
adj.results <- as.data.frame(bind_rows(adj.results))
adj.results <- cbind(Test, adj.results)


#Write up

write.table(adj.results, "output/FishersExactTestbyOrigin_adjustedpvalues.tsv", 
