#20th Nov 2020
#A script to run Fisher's exact test on unadjusted pvalues - solitary version
...
p.sol.results <- lapply(data.sol, fisher.craft)

#Combine into a table
#Record tests
Test <- rownames(p.sol.results[[1]])

#Unlist and add test names as first column
p.sol.results <- as.data.frame(bind_rows(p.sol.results))
p.sol.results <- cbind(Test, p.sol.results)


#Write up

write.table(p.sol.results, "output/FishersExactTestbyOrigin_Sol_unadjustedpvalues.tsv", 
